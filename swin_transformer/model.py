...
import collections
from .blocks import SwinTransformerStage, SwinTransformerStageV2
...
class SwinTransformer(tf.keras.Model):
    ...
    def flops(self):
        ...
        flops += self.num_features * self.config.nb_classes
        return flops


class SwinTransformerV2(tf.keras.Model):

    def __init__(self, config: ConfigDict, *args, **kwargs):
        super(SwinTransformerV2, self).__init__(*args, **kwargs)
        self.config = config
        self.norm_layer = norm_layer_factory(config.norm_layer)
        ...
        for idx in range(nb_stages):
            ...
            self.stages.append(
                SwinTransformerStageV2(
                    ...
                    drop_path_rate=dpr[block_idx_from:block_idx_to],
                    downsample=idx < nb_stages - 1,  # Don't downsample the last stage
                    pretrained_window_size = config.pretrained_window_sizes[idx],
                    name=f"layers/{idx}",
                )
            )
    ...
    def forward_features(self, x, training=False, return_features=False):
        ...
        block_idx = 0
        for stage_idx, stage in enumerate(self.stages):
            print(stage.input_size, stage.embed_dim, "input_size")
            x = stage(x, training=training, return_features=return_features)
            if return_features:
                x, stage_features = x
                for k in range(self.config.nb_blocks[stage_idx]):
